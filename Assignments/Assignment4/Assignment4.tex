...
\begin{enumerate}
  ...
  For the internal nodes, a construct similar to saturating adders will be used, where any output larger than 3 will be represented by \verb|1XX|, again to simplify the circuit for similar reasons to the above. The precision loss is not an issue as the setup does not care as long as it's larger than 3 always.
  
  For the root node, a circuit specifically to detect each output was made instead.

  Diagrams and derivations attached at the end of the document.

  \item Attached at the end of the document.
\end{enumerate}
